...
final class FirstTorchButton extends Button {
    ...
    protected void extractContents(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (kind == Kind.FOOTER) {
            ...
            label = label.copy().withStyle(style -> style.withShadowColor(0).withBold(false));
        }
        if (kind == Kind.SEARCH || kind == Kind.ACCESSIBILITY || kind == Kind.SETTINGS) {
            ...
            graphics.outline(x, y, 12, 10, FirstTorchTheme.AMBER);
            ...
            graphics.pose().translate(bounds.centerX() - iconSize / 2F, bounds.centerY() - iconSize / 2F);
            graphics.pose().scale(iconSize / 16F, iconSize / 16F);
            graphics.item(lessonIcon, 0, 0);
            graphics.pose().popMatrix();
            if (completed) {
                Rect badge = FirstTorchLayout.nodeCompletionBadge(bounds);
                graphics.pose().pushMatrix();
                graphics.pose().translate(badge.x(), badge.y());
                graphics.pose().scale(badge.width() / 16F, badge.height() / 16F);
