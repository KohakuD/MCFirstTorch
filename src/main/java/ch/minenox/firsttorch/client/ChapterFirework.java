...
final class ChapterFirework {
    ...
    static void draw(GuiGraphicsExtractor graphics, Font font, int width, int height) {
        ...
        int textY = cy + 49;
        for (var line : lines) {
            graphics.centeredText(font, line, cx, textY, (fade << 24) | 0xFFD174);
            textY += 10;
        }
        for (var line : font.split(Component.translatable(active.titleKey()), Math.max(1, Math.min(220, width - 30)))) {
            graphics.centeredText(font, line, cx, textY, (fade << 24) | 0xFFF1CA);
            textY += 10;
        }
    }
