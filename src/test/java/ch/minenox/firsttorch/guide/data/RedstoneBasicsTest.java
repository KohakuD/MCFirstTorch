...
final class RedstoneBasicsTest {
    ...
    @Test void optionalChapterOpensAfterMiningPreparationWithoutGatingEarlierLessons() throws Exception {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            assertEquals(List.of(i == 0 ? "1C4EA0627FB38D59" : quest(i)), chapter.quests().get(i).prerequisiteQuestIds());
        }
    ...
    @Test void materialChecksStayAutomaticAndCircuitProofStaysManual() throws Exception {
        ...
        var supplied = TaskEvaluator.evaluate(snapshot, new ProgressState(Set.of(), Set.of("1C4EA0627FB38D59")), key -> 64);
    ...
    @Test void miningPreparationRevealsOnlyTheFirstRedstoneChapter() throws Exception {
        var guide = snapshot().guides().getFirst();
        var before = new ch.minenox.firsttorch.network.ProgressPayload(ProgressState.EMPTY, java.util.Map.of(), true);
        var after = new ch.minenox.firsttorch.network.ProgressPayload(
                new ProgressState(Set.of(), Set.of("1C4EA0627FB38D59")), java.util.Map.of(), true);
        assertTrue(ch.minenox.firsttorch.client.ChapterVisibility.visibleChapters(guide, before).stream()
                .noneMatch(c -> c.titleKey().contains("redstone_")));
        var visible = ch.minenox.firsttorch.client.ChapterVisibility.visibleChapters(guide, after);
        assertEquals(List.of("chapter.firsttorch.redstone_basics.title"), visible.stream()
                .filter(c -> c.titleKey().contains("redstone_"))
                .map(c -> c.titleKey()).toList());
    }
}
