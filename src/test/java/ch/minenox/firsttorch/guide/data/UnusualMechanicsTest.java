...
final class UnusualMechanicsTest {
    ...
    @Test void twoCompactChaptersNeverGateEachOtherOrEarlierQuests() throws Exception {
        var chapters = snapshot().guides().getFirst().chapters();
        var additions = chapters.subList(62, 64);
        assertEquals(List.of("chapter.firsttorch.mechanics_blocks.title", "chapter.firsttorch.mechanics_names.title"), additions.stream().map(c -> c.titleKey()).toList());
    ...
    @Test void inventoryDoesNotReplaceReadingAndAnyCardCanBeFirst() throws Exception {
        ...
        var cards = guides.guides().getFirst().chapters().subList(62, 64).stream()
    ...
    @Test void twoBookTrophiesAreIndependentAndMatchChapterIcons() throws Exception {
        var guides = snapshot();
        var additions = guides.guides().getFirst().chapters().subList(62, 64);
        ...
        assertEquals(List.of(false, true), trophies.stream().map(TrophyCatalog.Entry::earned).toList());
        assertEquals(List.of("minecraft:white_concrete", "minecraft:name_tag"),
    ...
    @Test void bothLanguagesPreserveCaseSensitiveNamesAndSafetyDistinctions() throws Exception {
        for (var language : List.of("en_us", "de_de")) {
            try (var input = getClass().getResourceAsStream("/assets/firsttorch/lang/" + language + ".json")) {
                ...
                var honey = text.get("quest.firsttorch.field_special.bee.description").getAsString().toLowerCase(java.util.Locale.ROOT);
