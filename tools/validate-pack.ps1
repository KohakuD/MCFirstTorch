...
$expectedSmartFilters = @{
    ...
    '0166C28E593D07C5' = 'or(item(minecraft:iron_sword)item(minecraft:diamond_sword)item(minecraft:netherite_sword))'
    '1277D39F6A4E18D6' = 'or(item(minecraft:iron_pickaxe)item(minecraft:diamond_pickaxe)item(minecraft:netherite_pickaxe))'
    '56BB17D3AE825D1A' = 'or(item(minecraft:cooked_beef)item(minecraft:cooked_porkchop)item(minecraft:cooked_chicken)item(minecraft:cooked_mutton)item(minecraft:cooked_rabbit)item(minecraft:cooked_cod)item(minecraft:cooked_salmon)item(minecraft:baked_potato))'
    '562CE90B8374DF1A' = 'or(item(minecraft:cooked_beef)item(minecraft:cooked_porkchop)item(minecraft:cooked_chicken)item(minecraft:cooked_mutton)item(minecraft:cooked_rabbit)item(minecraft:cooked_cod)item(minecraft:cooked_salmon)item(minecraft:baked_potato)item(minecraft:dried_kelp))'
    '49E35072FAEB4681' = 'or(item(minecraft:oak_boat)item(minecraft:spruce_boat)item(minecraft:birch_boat)item(minecraft:jungle_boat)item(minecraft:acacia_boat)item(minecraft:dark_oak_boat)item(minecraft:mangrove_boat)item(minecraft:cherry_boat)item(minecraft:pale_oak_boat)item(minecraft:bamboo_raft))'
}
...
$expectedSingleItemTasks = @{
    '57C8EA2F3B6D9140' = 'minecraft:wooden_shovel'
    ...
    '3CB17D3904E8B270' = 'minecraft:bow'
    '2388E4A07B5F29E7' = 'minecraft:shield'
    '3499F5B18C603AF8' = 'minecraft:water_bucket'
    '451BD8FA7263CE09' = 'minecraft:shield'
}
...
$shovelDependencyPattern = 'id:\s*"46B7D91E2A5C803F"[\s\S]{0,180}?dependencies:\s*\["56CA245D80EB7913"\]'
Assert-True ([regex]::IsMatch($allDefinitionText, $shovelDependencyPattern)) 'The Wooden Shovel lesson must follow the Crafting Table lesson.'
$emergencyShelterDependencyPattern = 'id:\s*"1064CEF72A8513BD"[\s\S]{0,180}?dependencies:\s*\["46B7D91E2A5C803F"\]'
...
Assert-True ([regex]::IsMatch($allDefinitionText, $firstMorningDependencyPattern)) 'The first-morning recap must require both the Bed and the permanent shelter upgrade.'
$overworldUnlockPattern = 'id:\s*"12E8A5C74F309BD6"[\s\S]{0,260}?dependencies:\s*\[[\s\S]{0,100}?"0BED012A8146C359"[\s\S]{0,100}?"07B5E9C31D864AF2"'
Assert-True ([regex]::IsMatch($allDefinitionText, $overworldUnlockPattern)) 'The Overworld activities chapter must require both the route-home and protection recaps.'
$overworldBoatPathPattern = 'id:\s*"16B02D4FC7B8135E"[\s\S]{0,180}?dependencies:\s*\["34FAC7E96152BDF8"\][\s\S]*?id:\s*"38D24F61E9DA3570"[\s\S]{0,180}?dependencies:\s*\["16B02D4FC7B8135E"\][\s\S]*?id:\s*"5AF461830BFC5792"[\s\S]{0,180}?dependencies:\s*\["38D24F61E9DA3570"\][\s\S]*?id:\s*"7C1683A52D1E79B4"[\s\S]{0,180}?dependencies:\s*\["5AF461830BFC5792"\]'
...
$infiniteWaterPathPattern = 'id:\s*"50A42D7E18C6B39F"[\s\S]{0,180}?dependencies:\s*\["1D0F3A4C61E9B257"\]'
Assert-True ([regex]::IsMatch($allDefinitionText, $infiniteWaterPathPattern)) 'The infinite Water-source lesson must follow the first Water Bucket exercise.'
$ironEssentialsFinalPattern = 'id:\s*"3F215C6E03ABD479"[\s\S]{0,220}?dependencies:\s*\[[\s\S]{0,100}?"49CBF6082DA57E13"[\s\S]{0,100}?"50A42D7E18C6B39F"'
Assert-True ([regex]::IsMatch($allDefinitionText, $ironEssentialsFinalPattern)) 'The Iron Essentials recap must depend on both the Iron Pickaxe and completed Water-source paths.'
...
$endermanShelterPattern = 'id:\s*"0CB72D9F6A4E29E8"[\s\S]{0,180}?dependencies:\s*\["3D92E4A07B5F3AF8"\][\s\S]{0,180}?shape:\s*"gear"'
Assert-True ([regex]::IsMatch($allDefinitionText, $endermanShelterPattern)) 'The optional Enderman shelter must branch from the Water retreat.'
$endermanShelterDependencyPattern = 'dependencies:\s*\[[^\]]*"0CB72D9F6A4E29E8"'
Assert-True (-not [regex]::IsMatch($allDefinitionText, $endermanShelterDependencyPattern)) 'The optional Enderman shelter must not gate another quest.'
$endCrystalPathPattern = 'id:\s*"5FA406C29D715C1B"[\s\S]{0,180}?dependencies:\s*\["4EA3F5B18C604B09"\][\s\S]*?id:\s*"60B517D3AE826D2C"[\s\S]{0,180}?dependencies:\s*\["5FA406C29D715C1B"\][\s\S]*?id:\s*"71C628E4BF937E3D"[\s\S]{0,180}?dependencies:\s*\["60B517D3AE826D2C"\]'
Assert-True ([regex]::IsMatch($allDefinitionText, $endCrystalPathPattern)) 'End Crystal removal must remain ordered after battlefield orientation.'
$dragonFightPathPattern = 'id:\s*"12D739F5C0A48F4E"[\s\S]{0,180}?dependencies:\s*\["71C628E4BF937E3D"\][\s\S]*?id:\s*"34F95B17E2C6B170"[\s\S]{0,180}?dependencies:\s*\["12D739F5C0A48F4E"\][\s\S]*?id:\s*"561B7D3904E8D392"[\s\S]{0,180}?dependencies:\s*\["34F95B17E2C6B170"\]'
Assert-True ([regex]::IsMatch($allDefinitionText, $dragonFightPathPattern)) 'The Ender Dragon flight, perched, and victory lessons must remain ordered after Crystal removal.'
$dragonKillPattern = 'id:\s*"672C8E4A15F9E4A3"[\s\S]{0,180}?type:\s*"advancement"[\s\S]{0,120}?advancement:\s*"minecraft:end/kill_dragon"[\s\S]{0,120}?criterion:\s*"killed_dragon"'
...
Assert-True (-not [regex]::IsMatch($allDefinitionText, $dragonEggDependencyPattern)) 'The optional Dragon Egg must not gate another quest.'
$outerEndPathPattern = 'id:\s*"6794EFB18C065B1B"[\s\S]{0,180}?dependencies:\s*\["4572CD9F6AE439F9"\][\s\S]*?id:\s*"3BD823F5C04A9F5F"[\s\S]{0,180}?dependencies:\s*\["6794EFB18C065B1B"\][\s\S]*?id:\s*"5DFA4517E26CB171"[\s\S]{0,180}?dependencies:\s*\["3BD823F5C04A9F5F"\][\s\S]*?id:\s*"7F1C6739048ED393"[\s\S]{0,180}?dependencies:\s*\["5DFA4517E26CB171"\][\s\S]*?id:\s*"213E895B26A0F5B5"[\s\S]{0,180}?dependencies:\s*\["7F1C6739048ED393"\]'
Assert-True ([regex]::IsMatch($allDefinitionText, $outerEndPathPattern)) 'The second End visit must remain ordered through the secured outer-island arrival.'
$chorusPathPattern = 'id:\s*"435FA7C29D816E3B"[\s\S]{0,180}?dependencies:\s*\["213E895B26A0F5B5"\][\s\S]*?id:\s*"7682DAF5C0B4916E"[\s\S]{0,180}?dependencies:\s*\["435FA7C29D816E3B"\]'
Assert-True ([regex]::IsMatch($allDefinitionText, $chorusPathPattern)) 'Chorus harvesting and the controlled first teleport must follow the secured outer-island arrival in order.'
$chorusFruitTaskPattern = 'id:\s*"5460B8D3AE927F4C"[\s\S]{0,120}?type:\s*"item"[\s\S]{0,120}?count:\s*8\s*,[\s\S]{0,160}?item:\s*\{\s*id:\s*"minecraft:chorus_fruit"'
Assert-True ([regex]::IsMatch($allDefinitionText, $chorusFruitTaskPattern)) 'The Chorus harvest must require eight Fruit simultaneously.'
$chorusFlowerTaskPattern = 'id:\s*"6571C9E4BFA3805D"[\s\S]{0,120}?type:\s*"item"[\s\S]{0,160}?item:\s*\{\s*id:\s*"minecraft:chorus_flower"'
Assert-True ([regex]::IsMatch($allDefinitionText, $chorusFlowerTaskPattern)) 'The Chorus harvest must preserve one directly collected Flower.'
$endGatewayPattern = 'id:\s*"102D784A159FE4A4"[\s\S]{0,180}?type:\s*"advancement"[\s\S]{0,120}?advancement:\s*"minecraft:end/enter_end_gateway"[\s\S]{0,120}?criterion:\s*"entered_end_gateway"'
Assert-True ([regex]::IsMatch($allDefinitionText, $endGatewayPattern)) 'The first End Gateway transfer must use the exact Vanilla entered-End-Gateway criterion.'
$outerEndPearlPattern = 'id:\s*"08A5F0C29D176C2C"[\s\S]{0,120}?type:\s*"item"[\s\S]{0,120}?count:\s*4\s*,[\s\S]{0,160}?item:\s*\{\s*id:\s*"minecraft:ender_pearl"\s*,\s*count:\s*1\s*\}'
Assert-True ([regex]::IsMatch($allDefinitionText, $outerEndPearlPattern)) 'The second End visit must require four Ender Pearls for both directions and reserves.'
$endStoneTaskPattern = 'id:\s*"35CA6C28F3D7C271"[\s\S]{0,120}?type:\s*"item"[\s\S]{0,120}?count:\s*64\s*,[\s\S]{0,160}?item:\s*\{\s*id:\s*"minecraft:end_stone"\s*,\s*count:\s*1\s*\}'
Assert-True ([regex]::IsMatch($allDefinitionText, $endStoneTaskPattern)) 'The cage-opening lesson must require 64 End Stone at task level.'
...
$firstStepsLanguageText = ((Get-ChildItem -LiteralPath (Join-Path $questRoot 'lang') -Recurse -File -Filter 'first_steps.json5' | ForEach-Object { Get-Content -LiteralPath $_.FullName -Raw }) -join "`n")
foreach ($imageName in @('wooden_shovel.png', 'first_night_shelter.png')) {
    $referenceCount = [regex]::Matches($firstStepsLanguageText, [regex]::Escape("firsttorch:textures/questpics/$imageName")).Count
    Assert-True ($referenceCount -eq 2) "The $imageName guide must be referenced once in each First Steps language file."
}
...
foreach ($imageName in @('stronghold_silverfish.png', 'end_portal_room_capture.png', 'end_portal_frame_states.png', 'stronghold_iron_door_capture.png')) {
...
foreach ($imageName in @('bow_and_arrows.png', 'end_portal_active_capture.png')) {
    $referenceCount = [regex]::Matches($strongholdLanguageText, [regex]::Escape("firsttorch:textures/questpics/$imageName")).Count
    Assert-True ($referenceCount -eq 2) "The $imageName guide must be referenced once in each Stronghold language file."
}
...
$endArrivalImageReferences = [regex]::Matches($endLanguageText, 'firsttorch:textures/questpics/end_arrival_platform_capture\.png')
...
Assert-True ($endermanShelterImageReferences.Count -eq 2) 'The optional Enderman shelter guide must be referenced once in each language.'
$endBattlefieldImageReferences = [regex]::Matches($endLanguageText, 'firsttorch:textures/questpics/end_battlefield\.png')
Assert-True ($endBattlefieldImageReferences.Count -eq 2) 'The End battlefield screenshot must be referenced once in each language.'
$endCrystalExposedImageReferences = [regex]::Matches($endLanguageText, 'firsttorch:textures/questpics/end_crystal_exposed\.png')
Assert-True ($endCrystalExposedImageReferences.Count -eq 2) 'The exposed End Crystal guide must be referenced once in each language.'
$endCrystalImageReferences = [regex]::Matches($endLanguageText, 'firsttorch:textures/questpics/end_crystal_removal\.png')
Assert-True ($endCrystalImageReferences.Count -eq 2) 'The End Crystal removal guide must be referenced once in each language.'
foreach ($imageName in @('ender_dragon_flight.png', 'ender_dragon_perched.png', 'end_exit_portal.png', 'dragon_egg_retrieval.png', 'end_gateway_access.png', 'outer_end_arrival.png')) {
    $referenceCount = [regex]::Matches($endLanguageText, [regex]::Escape("firsttorch:textures/questpics/$imageName")).Count
    Assert-True ($referenceCount -eq 2) "The $imageName guide must be referenced once in each End language file."
}
...
$infiniteWaterImageReferences = [regex]::Matches(((Get-ChildItem -LiteralPath (Join-Path $questRoot 'lang') -Recurse -File -Filter '*.json5' | ForEach-Object { Get-Content -LiteralPath $_.FullName -Raw }) -join "`n"), 'firsttorch:textures/questpics/infinite_water_sources\.png')
Assert-True ($infiniteWaterImageReferences.Count -eq 2) 'The infinite Water-source guide must be referenced once in each language.'
