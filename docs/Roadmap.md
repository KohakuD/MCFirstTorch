...
### Previous baseline acceptance and publication history
...
- Historical 0.13 publication task: superseded by the prepared 0.14.0 packages. The old submission history below is retained; it is not a request to upload an obsolete JAR.
  ...
  - The corrected 0.13 submission/public availability was not confirmed in this record. Do not submit it now; use the 0.14.0 packages and publication checklist below.
...
- [x] Complete the native API, loader and data/resource-format port assessment.
  ...
  - [x] Implement production entry/client-handler registration, native client UI/renderers and initial target-specific resources; compile/test and client startup checks pass. Live gameplay acceptance is recorded below.
...
- [x] Backport the native runtime and review every lesson against `1.21.1` mechanics, recipes, objectives and available content. Maintain English and German together and use exact target-version game assets.
  - [x] Adapt known incompatible items, Iron armour, Netherite lodestone and optional excursion prerequisites in both languages; include the owner-approved shared captures unchanged. The owner accepted the implemented mechanics and visuals.
...
- [x] Validate and prepare the `1.21.1` edition before proceeding to the `26.2` port; the owner confirmed its upload and authorised proceeding. Public availability is tracked separately. A mod backport does not imply support for downgrading existing Minecraft worlds or importing progress across versions.
...
- [x] Record official prerequisites and implement an initial isolated `:mc262` native build. Java 25 / NeoForge 26.2.0.86, five client adapters, all 417 reused target tests and package verification pass. Owner accepted the requested bilingual gameplay/UI smoke checks after the metadata correction. Reviewed gameplay content is implemented and owner-accepted. See `Port262.md`.

- [x] Port the native runtime, dependencies and resources to `26.2` while preserving support for the earlier targets.
- [x] Review the Minecraft changes since `26.1.2`, add new lessons and update affected existing lessons in both languages.
  - [x] Add five optional bilingual Sulfur Caves cards, links and a reading trophy. Friends List guidance and the expanded Sulfur cards/trophy are owner-accepted.
...
- [x] Run focused source/data tests, the full test/build gate and native JAR verification for each installable target artifact. Latest code baseline: `a7d5277`, 1,106 tests, no failures/errors/skips; all three JAR boundary checks passed.
- [x] Test each target with fresh profiles in English and German: welcome choices, baseline selection, cumulative comparison results, all-quests switching, prerequisite access, tasks, rewards and persistence after restart.
- [x] Verify that each target packages only compatible content and original target-version assets, and that normal `26.1.2` behaviour remains intact. Native illustrations use installed target resources; the separately documented village-image provenance exception remains unchanged.
...
- Dedicated-server operation and Vulkan remain outside the accepted test scope. Live Friends joining is not claimed as separately verified by the implemented-content acceptance.
...
- [x] Owner accepted all four 26.1 cards as working and correct on 2026-09-13. Subsequent remediation blocks are completed below; historical detail remains in CurriculumCoverageAudit.md.
...
- [x] Accept the expanded Trials block in game in both languages; complete the later-update blocks before release.
...
- [x] Finish Copper tool/building-use coverage, review Sulfur/Friends mechanics, and complete remaining target-specific release checks. See CurriculumCoverageAudit.md for the review record.

- [x] Extend the Copper Ingot lesson with optional tool crafting and mining-tier guidance in 26.1.2/26.2; preserve its inventory task and the 1.21.1 text. Owner reading/layout acceptance is confirmed.
...
- [x] Accept the Building and Lighting cards in game in both languages; continue the Sulfur/Friends detail review.

- [x] Review and expand the existing 26.2 Sulfur and Friends instructions using native data/code and official release notes.
- [x] Accept the expanded bilingual readings, links and Sulfur trophy; live Friends joining remains unverified.
