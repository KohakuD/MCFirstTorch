...
class FirstTorchBrowserScreen extends Screen {
    ...
    private boolean testCompletionAvailable() {
        ...
        return ch.minenox.firsttorch.guide.progress.DevelopmentQuestAccess.allowed(
                ...
                owner == null ? null : owner.getId(), minecraft.player.getUUID());
    }
    ...
    private void addHeaderControls() {
        ...
        if (!preview() && !references.isEmpty()) {
            ...
            addRenderableWidget(button(x, y + size + 4, size, size, Component.empty(),
                    ignored -> minecraft.setScreen(new FirstTorchReferenceScreen(this, references, this::openReferenceChapter)),
        ...
        addRenderableWidget(button(x, y, size, size, Component.empty(),
                ignored -> minecraft.setScreen(new FirstTorchSearchScreen(this, query, this::search)),
        ...
        addRenderableWidget(button(x + size + 4, y + size + 4, size, size, Component.empty(),
                ignored -> minecraft.setScreen(new FirstTorchAccessibilityScreen(this, minecraft.options)),
    ...
    private ItemStack icon(String objectId) {
        ...
        return new ItemStack(BuiltInRegistries.ITEM.get(ResourceLocation.parse(DesignPreview.itemId(objectId))));
    }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.pose().translate(viewport.x(), viewport.y(), 0);
        ...
        drawText(graphics);
        positionReferenceButtons();
        super.render(graphics, (int) viewport.localX(mouseX), (int) viewport.localY(mouseY), partialTick);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        double localX = viewport.localX(mouseX), localY = viewport.localY(mouseY);
        Rect track = chapterTrack();
        if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT && chapterRowCount > chapterCapacity
                && contains(track, localX, localY)) {
            ...
            chapterDragOffset = localY >= top && localY < top + height ? localY - top : height / 2.0;
            draggingChapters = true;
            dragChapters(localY);
            return true;
        }
        return super.mouseClicked(viewport.localX(mouseX), viewport.localY(mouseY), button);
    }

    @Override
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        if (draggingChapters && button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
            draggingChapters = false;
            return true;
        }
        return super.mouseReleased(viewport.localX(mouseX), viewport.localY(mouseY), button);
    }

    @Override
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dx, double dy) {
        if (draggingChapters) {
            dragChapters(viewport.localY(mouseY));
            return true;
        }
        return super.mouseDragged(viewport.localX(mouseX), viewport.localY(mouseY), button, dx / viewport.scale(), dy / viewport.scale());
    }
    ...
    private void drawChapterScrollbar(GuiGraphics graphics) {
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (chapterButtons.contains(getFocused())) {
            ...
            if (keyCode == GLFW.GLFW_KEY_PAGE_UP || first >= 0) {
    ...
    private static void drawArrow(GuiGraphics graphics, Rect source, Rect target) {
    ...
    private static void drawLine(GuiGraphics graphics, int x, int y, int targetX, int targetY) {
    ...
    private void drawMasthead(GuiGraphics graphics, Rect top) {
        ...
        graphics.pose().pushPose();
        graphics.pose().translate(x, y + 2, 0);
        graphics.pose().scale(torchSize / 16F, torchSize / 16F, 1);
        graphics.renderItem(brandTorch, 0, 0);
        graphics.pose().popPose();
        int flameWidth = Math.max(10, torchSize / 2);
        graphics.blit(x + (torchSize - flameWidth) / 2, y, 0, flameWidth, Math.max(13, torchSize * 3 / 5), ModelBakery.FIRE_0.sprite());
        ...
        graphics.pose().pushPose();
        graphics.pose().translate(x + torchSize + 9, y + (compact ? 4 : 7), 0);
        graphics.pose().scale(scale, scale, 1);
        new FirstTorchText(graphics).accept(1, 1, colored(brand, FirstTorchTheme.GOLD_DARK));
        new FirstTorchText(graphics).accept(0, 0, colored(brand, FirstTorchTheme.AMBER));
        graphics.pose().popPose();
    }

    private void drawProgress(GuiGraphics graphics, FirstTorchText text, Rect top) {
        boolean compact = top.width() < 600;
    ...
    private void drawEmptyState(FirstTorchText text) {
        Rect panel = layout.questMap();
    ...
    private void drawDetails(GuiGraphics graphics, FirstTorchText text) {
        Rect panel = layout.details();
        ...
        if (w >= 150) {
            ...
            graphics.renderItem(icon(viewModel.quest().id()), badge.centerX() - 8, badge.centerY() - 8);
    ...
    private int drawWrapped(FirstTorchText text, Component component, int x, int y, int maxWidth, int maxRows, int color) {
