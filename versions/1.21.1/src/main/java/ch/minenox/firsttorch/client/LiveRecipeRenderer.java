...
final class LiveRecipeRenderer {
    ...
    static void drawPanels(GuiGraphics graphics, FirstTorchLayout.Rect bounds,
    ...
    static void draw(GuiGraphics graphics, FirstTorchLayout.Rect bounds, LiveRecipeCatalog.Recipe recipe) {
    ...
    static void slot(GuiGraphics graphics, int x, int y, String item) {
        graphics.fill(x, y, x + 42, y + 42, FirstTorchTheme.BACKGROUND);
        graphics.renderOutline(x, y, 42, 42, FirstTorchTheme.MUTED);
        if (item.isEmpty()) return;
        graphics.pose().pushPose();
        graphics.pose().translate(x + 5, y + 5, 0);
        graphics.pose().scale(2F, 2F, 1F);
        graphics.renderItem(QuestIcons.resolveItem(item), 0, 0);
        graphics.pose().popPose();
    }
}
